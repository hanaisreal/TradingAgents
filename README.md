...
### Realtime-like Paper Trading

TradingAgents also includes a safe paper-trading loop for rehearsing automated trading before any live brokerage integration. It never places real orders. It polls quotes/news/social sources, writes a JSON paper ledger, and updates the dashboard status file so you can watch the portfolio like a live run.

```bash
tradingagents-paper \
  --tickers 005930.KS,247540.KQ \
  --cash 10000000 \
  --cash-per-trade 1000000 \
  --interval 30 \
  --iterations 0
```

Runtime files are intentionally local-only and ignored by git:
- `.paper_trading/state.json`: paper cash, positions, and the latest order ledger.
- `dashboard/status.json`: dashboard feed generated by the paper loop.

Fast data sources used by the paper loop:
- Quotes: yfinance `fast_info`/1-minute history fallback. Korean quotes may be exchange-delayed unless a broker realtime feed is added.
- News: Naver Search API when configured, Google News RSS fallback, and yfinance news.
- Social/community: X recent search when `X_BEARER_TOKEN` or `TWITTER_BEARER_TOKEN` is configured. Existing StockTwits/Reddit fetchers remain available for US-style ticker discussion context.

Optional realtime/social keys:
```bash
NAVER_CLIENT_ID=...
NAVER_CLIENT_SECRET=...
X_BEARER_TOKEN=...
...
For actual live Korean brokerage execution, add a broker adapter such as Korea Investment Securities or Kiwoom only after paper-trading validation, explicit manual approval, order-size caps, duplicate-order protection, and a kill switch are in place.
